...
func Run(ctx context.Context, db *pgxpool.Pool, log *zap.Logger) error {
	...
	if err != nil {
		return fmt.Errorf("failed to query schema migrations: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&version); err != nil {
			return fmt.Errorf("failed to scan schema migrations: %w", err)
		}
	...
	for _, file := range files {
		...
		log.Info("Running migrations", zap.String("file", file))
